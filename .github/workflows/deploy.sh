...
check_schema_compatibility() {
  ...
  if [[ -z "${SQLITE_MIGRATION_APPROVAL:-}" || -z "${SQLITE_RECOVERY_PLAN:-}" ]]; then
    echo "SQLite schema changed ($current -> $candidate); reviewed SQLITE_MIGRATION_APPROVAL and SQLITE_RECOVERY_PLAN are required" >&2
    return 1
  fi
  echo "Reviewed SQLite migration approval $SQLITE_MIGRATION_APPROVAL accepted with recovery plan $SQLITE_RECOVERY_PLAN"
}
